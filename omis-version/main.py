...
# get position of the added text

pos_x = int(config.get('dab-broadcast', 'textpos_x'))
pos_y = int(config.get('dab-broadcast', 'textpos_y'))
rel_x = pos_x + int(config.get('dab-broadcast', 'textrel_x'))
rel_y = pos_y + int(config.get('dab-broadcast', 'textrel_y'))
    ...
    if songname != last_songname or eas_status_changed:
        ...
        if emergency_message:
            ...
            for line in lines:
                ...
                y_text += EMtext.getsize(line)[1] # I dont really care about EAS Okay
        else:
            image = Image.open(broadcast2_image_path)
            draw = ImageDraw.Draw(image)
            draw.text((pos_x, pos_y), songname, fill="white", font=songfont)
            draw.text((rel_x, rel_y), "by " + artistname, fill="white", font=artistfont)
